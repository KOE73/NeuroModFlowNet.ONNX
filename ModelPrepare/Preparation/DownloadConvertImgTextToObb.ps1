...
$SourceBaseUrl = if ([string]::IsNullOrWhiteSpace($AssetsBaseUrl)) { $GitAssetsBaseUrl } else { $AssetsBaseUrl }
...
function Ensure-SourceModel {
    ...
    $url = "${SourceBaseUrl}${ModelName}.pt"
    Write-Host "Downloading project OBB model $ModelName from $url" -ForegroundColor Green
    Invoke-WebRequest -Uri $url -OutFile $sourceModelPath -ErrorAction Stop
    return $sourceModelPath
}

function Convert-ProjectObbModel {
    param(
        [string] $ModelName,
    ...
    if (!(Test-Path $onnxPath)) {
        ...
        Write-Host "`n>>> Exporting project OBB model: $onnxPath" -ForegroundColor Green
...
foreach ($model in $Models) {
    ...
    if ($UseDynamic) {
        Convert-ProjectObbModel $modelName $sourceModelPath 1
        continue
    }

    foreach ($batch in $Batches) {
        Convert-ProjectObbModel $modelName $sourceModelPath $batch
    }
}
