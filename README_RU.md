...
- `tools/NeuroModFlowNet.ONNX.Tools` — инструменты для просмотра и модификации ONNX-моделей.

## Tools

`NeuroModFlowNet.ONNX.Tools` — отдельный CLI и библиотека для работы с ONNX-моделями на уровне графа. Он используется для просмотра структуры модели, входов, выходов и атрибутов, а также для более глубоких преобразований, например добавления preprocessing-голов или подготовки модели под конкретный backend.

На практике этот слой важен не меньше runtime-кода: если preprocessing удается перенести внутрь модели, внешний pipeline становится проще и быстрее.
