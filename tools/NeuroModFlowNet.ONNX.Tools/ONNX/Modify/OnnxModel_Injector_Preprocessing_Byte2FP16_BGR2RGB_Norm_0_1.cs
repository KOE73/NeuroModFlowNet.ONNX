...
[OnnxHead("ByteBGR_FP16", "Standard NHWC Byte BGR input head to FP16")]
public class OnnxModel_Injector_Preprocessing_Byte2FP16_BGR2RGB_Norm_0_1 : OnnxModelModifier
{
    public override string GetDefaultExtraName() => "_head";

    // 1. ОПРЕДЕЛЯЕМ ИМЕНА ТЕНЗОРОВ
    const string New_RawByteBGR_InputName = "pixel_values";                             // U8 NHWC BGR
    const string Step1_CastToFP16_FP16_BGR_NHWC_OutputName = "pixels_fp16_bgr_nchw";    // FP16 NCHW RGB
    const string Step2_BGR2RGB_FP16_RGB_NHWC_OutputName = "pixels_fp16_nhwc";           // U8 NHWC RGB
    const string Step3_Transpose_FP16_RGB_NCHW_OutputName = "pixels_fp16_rgb_nhwc";     // U8 NCHW RGB

    const string ByteBGR_FP16_cast_fp16 = "ByteBGR_FP16_cast_fp16";
    const string ByteBGR_FP16_bgr_to_rgb = "ByteBGR_FP16_bgr_to_rgb";
    const string ByteBGR_FP16_transpose_nhwc_to_nchw = "ByteBGR_FP16_transpose_nhwc_to_nchw";
    const string ByteBGR_FP16_norm = "ByteBGR_FP16_norm";
    ...
    public override void Inject(ModelProto model)
    {
        ...
        {
            Name = ByteBGR_FP16_cast_fp16,
            OpType = "Cast",
            Input = { New_RawByteBGR_InputName },
            Output = { Step1_CastToFP16_FP16_BGR_NHWC_OutputName },
        ...
        {
            Name = ByteBGR_FP16_bgr_to_rgb,
            OpType = "Gather",
            Input = { Step1_CastToFP16_FP16_BGR_NHWC_OutputName, gatherIndicesName },
            Output = { Step2_BGR2RGB_FP16_RGB_NHWC_OutputName },
        ...
        {
            Name = ByteBGR_FP16_transpose_nhwc_to_nchw,
            OpType = "Transpose",
            Input = { Step2_BGR2RGB_FP16_RGB_NHWC_OutputName },
            Output = { Step3_Transpose_FP16_RGB_NCHW_OutputName },
        ...
        // 4.1 Создаем константу 0.00392157, Записываем Half значение (0.00392157) в байтовом виде
        ...
        var scaleIniter = new TensorProto
        {
            ...
            Dims = { 1 },
            RawData = ByteString.CopyFrom(BitConverter.GetBytes((Half)(1f / 255f)))
        };
        ...
        {
            Name = ByteBGR_FP16_norm,
            OpType = "Mul",
            Input = { Step3_Transpose_FP16_RGB_NCHW_OutputName, normScaleName },
        ...
        [
            // После Cast: [N, H, W, C]
            CreateValueInfo(Step1_CastToFP16_FP16_BGR_NHWC_OutputName, TensorProto.Types.DataType.Float16, origDim0Batch, origDim2Height, origDim3Width, origDim1Channels),
            
            // После Gather: [N, H, W, C]
            CreateValueInfo(Step2_BGR2RGB_FP16_RGB_NHWC_OutputName, TensorProto.Types.DataType.Float16, origDim0Batch, origDim2Height, origDim3Width, origDim1Channels),
            
            // После Transpose: [N, C, H, W]
            CreateValueInfo(Step3_Transpose_FP16_RGB_NCHW_OutputName, TensorProto.Types.DataType.Float16, origDim0Batch, origDim1Channels, origDim2Height, origDim3Width),
            
            // Описываем "images", который теперь не вход, а результат Mul
            CreateValueInfo(origInputFirstConvName, TensorProto.Types.DataType.Float16, origDim0Batch, origDim1Channels, origDim2Height, origDim3Width)
        ]);
    ...
    //// Хелпер для создания ValueInfo
    //protected ValueInfoProto CreateValueInfo_(
    //    string name,
